...
module PageHelper
  ...
  module ClassMethods
    ...
    def self.node(name, *args, &definition)
      module_eval do
...
class BrowserPlayground
  ...
  def self.define(method_name, definition)
    watir_element, how, what = definition
  ...
  def self.facea(name, *args, &definition)

    class_eval do
      define_method("#{name}_element") do |*args|
        lambda do |*args|
          definition
        end
      end
    ...
    class_eval "def #{name}(*args)
                  browser.send(#{name}_element.call(*args))
                end"

  end
  #define :bla, [:text_field, :name, "last_name0"]

  def self.define1(method_name, args=nil, definition=nil)
    watir_element, how, what = definition
    class_eval "def #{method_name}(*args)
        browser.#{watir_element}(:#{how}, \"#{what}\")
      end"
  ...
  #  define1 :bla do |*args|
  #    [:text_field, :name, "last_name#{args}"]
  #  end


  # what does this form buy me? hmm...
  #face(:last_0) { browser.text_field(:name, "last_name0") }
  node(:last_0) { browser.text_field(:name, "last_name0")}
  face :last_0f, [:text_field, :name, 'last_name0']
...
result = Benchmark.bmbm do |test|
  ...
  test.report('class eval block') do
    class PageByClassEval
      ...
      face(:last_0) { text_field(:name, "last_name0") }
      #face(:last_arg) { |i| browser.text_field(:name, "last_name#{i}") }
    end
    10000.times do
